...
마이그레이션은 동시 실행을 막지 않습니다. 두 요청이 같은 `feeds_v{n}`을 읽으면 같은 대상 인덱스 이름을 계산하고, 한쪽이 alias를 전환한 뒤 다른 쪽이
`createIndex`에서 그 인덱스를 지울 수 있습니다. 수동 트리거이고 ADMIN 권한이 필요해 발생 가능성은 낮지만, 트리거 전에 다른 마이그레이션이 진행 중이 아닌지
확인하세요.
